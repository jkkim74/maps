...
**✅ 1~9번 수정 완료(8/29). 🔴 10번만 남았다 — 이걸 풀기 전에는 `automatic` 으로 바꾸지 말 것.**
...
| 10 | 매도 우선 직렬 큐에 호출자가 없다 | 🔴 미해결 — 브로커 호출이 이벤트 루프에서 동기 실행, KIS 지연이 웹소켓·손절·EOD 훅을 함께 멈춤. asyncio 구조 변경이라 별도 작업 |
...
검증: 전체 **1,093 passed**. 1~9 각각에 실패 시나리오 테스트를 붙였다 — 취소·체결 경합,
