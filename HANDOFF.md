...
## 8/25 최우선 개발 항목 승격 — 🔴 보유 장세 오버레이 (계획 저장·미착수)

사용자 결정에 따라 **현재 장세와 매수 당시 장세를 비교하는 보유 장세 오버레이**를 다음 개발
작업의 최우선 순위로 올렸다. 기존의 “미완성 후보 점수 순위 격리”보다 먼저 진행한다.
구현 계획 정본은 아래 문서다.
...
- 설정은 `off|shadow|enforce`, 기본은 **`shadow`**다. shadow에서는 판정·감사·다이제스트만
  남기고 주문을 내지 않는다. 실제 운영 `enforce` 전환은 회귀 테스트, 최근 20거래일 재생,
  shadow 결과 검토와 **사용자의 명시적 승인 후에만** 한다.
...
계획은 순수 판정기 → 감사 테이블/Alembic 0028 → 자동후보 보유 연결 →
다이제스트/운영문서 → 전체 회귀·shadow 인계의 5개 TDD 작업으로 나눴다. **코드 구현,
마이그레이션, 배포는 아직 시작하지 않았고 현재 운영 동작은 변하지 않았다.**
