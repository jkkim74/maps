...
>
> ### 목표
> "오늘 아모텍 같은 종목이 왜 관찰 종목이 안 됐나" — 로그·DB·KIS 실측으로 원인을 잡고 고쳤다.
>
> ### 진단 — 게이트 탈락이 아니라 **후보 목록에 오르지 못했다**
>
> | 관측 | 값 |
> |---|---|
> | 아모텍 당일 | 시가 +0.3% → 09:10~09:15 봉에서 +25% 돌파 → 09:22 상한가 15,130(+29.98%) 잠김. 09:42 누적 184만 주 |
> | 스캔 로그 | 09:10:04 `순위 30건 → 후보 0건, 탈락 0건` 이후 종일 불변 |
> | `scan_rejections` 9/11 · `limit_up_session` 9/11 | 둘 다 비어 있음 — +25% 종목이 게이트까지 온 적 없음 |
> | 052710 메타 | KOSDAQ / STOCK / 상장 2003-08-01 — 자격은 통과했을 종목 |
> | KIS **거래량순위** 실측(09:44) | 30행, 30위 하한 **290만 주**, 상위권 KODEX/TIGER 인버스·레버리지 ETF·ETN. +25% **0건**, 052710 없음 |
> | KIS **등락률순위** 실측(09:45) | `FID_RSFL_RATE1=25, RATE2=100` 구간 → 7~9행 전부 +25%, **아모텍 1위**. 모의 도메인에서 정상 응답 |
>
> 구조적 편향: 일찍 잠기는 상한가일수록 거래량이 적어 거래량순위에서 밀린다. 9/7 이후 감시가 걸린
> 012210·004090·003280 은 전부 거래량이 큰 종목이었다 — 전략이 원하는 종목과 반대.
>
> ### 수정 — `kis_adapter.get_limit_up_candidates()` 후보 소스를 등락률순위(+25~100% 구간)로 교체
>
> 호출부 계약 `(rows, ranked_count)` 는 그대로. 코드 안 `≥25` 필터와 종목당 `inquire-price`(상한가·상장주수·정지)도 그대로.
> 실측으로 확정한 함정 3개를 테스트가 고정한다(`test_limit_up_candidate_scan_uses_rank_then_broker_quote`):
> ① 정렬 코드는 문서의 `"0000"` 이 아니라 `"0"`(`OPSQ2002 INVALID INPUT_FILED_SIZE [4]`),
> ② 구간 하한만 주면 0행 — 상·하한 둘 다 필수, ③ 필터 없이 부르면 30행이 등락률순이 아니다(프리티 +29.89% 누락).
> `ranked_count` 는 이제 구간 응답 행 수라 "오늘 +25% 없음"과 빈 응답이 같아 보인다(`rt_cd != 0` 은 여전히 예외). 30행 상한.
> 문서: `maps/limit_up/CLAUDE.md` 스캔 절. 마이그레이션·requirements 변경 없음.
>
> ### 배포 ① `1c6dd49` (09:58 KST) → 두 번째 원인 드러남 — `iscd_stat_cls_code=57` 을 정지로 판정
>
> 09:59:32 첫 스캔 `순위 7건 → 후보 7건, 신규감시 0건, 탈락 7건 (halted=7)`. `scan_rejections` 에 052710 포함 7건 전부 `halted`.
> 운영 `inquire-price` 실측: 아모텍 `temp_stop_yn=N, vi_cls_code=N, mrkt_warn_cls_code=00` 인데 **`iscd_stat_cls_code=57`**.
> 코드표(KIS 주식현재가): 00 그외 · 51 관리 · 52 투자위험 · 53 투자경고 · 54 투자주의 · 55 신용가능 · **57 증거금 100%** · 58 거래정지 · 59 단기과열.
> `_quote_is_halted()` 가 `{00, 55}` 외 전부를 정지로 봐서, 증거금 100%(상한가 소형주 대부분)가 전원 탈락했다.
> 오늘 7건 중 5건이 57(아모텍·카티스·샌즈랩·진영·알파칩스), 2건은 51 관리종목(케이엠제약·프리티 — 이건 정상 탈락).
> 9/10 종일 붙어 있던 `halted=1`(흥구석유 024060)도 57 이었다. 9/7~9/10 감시가 걸린 004090·003280 은 55.
> **수정 ②**: 허용 집합에 57 추가. 테스트 `test_halted_and_flagged_stocks_are_marked_for_the_scanner` 가 57 허용·58 차단을 고정.
> 남는 어긋남(기록만): 51~54·59 도 로그·`scan_rejections` 에 `halted` 로 찍힌다 — 관리종목이 "정지"로 보인다. 라벨 분리는 별건.
>
> ### 배포 ② `34301b5` (10:07 KST) — ✅ 실효 확인
> 10:07:37 첫 스캔 `순위 10건 → 후보 10건, 신규감시 6건, 탈락 4건 (halted=3, ineligible_security:preferred=1)`.
> `limit_up_session` #5~#10 = 052710(아모텍)·140430·411080·285800·117670·900270, 전부 `watching / automatic`.
> 아모텍 트리거가 15,100(상한가 15,130). **8/29 가동 이후 한 스캔에 6건 감시는 처음** — 후보 소스와 정지 판정 둘 다 실효.
> `FIRE_NET` 실주문은 10:08 현재 아직 없음(9/7 이월 항목 계속). 남은 `halted=3` 은 51 관리종목 2건 + 1건 미확인.
> 전체 테스트 `1240 passed` × 2회. 마이그레이션·requirements 없음. 두 배포 모두 `analyze` 미실행 확인 후.
>
> ### 매수 미발생 조사 (11:00~11:10) — 오류 아님, 진입 게이트
>
> 10:07 이후 감시 10건(#5~#14) 전부 `watching`, 이벤트·주문레그·order_log 0건, 가드 열림, ERROR 없음.
> 진입 조건(전부 AND): 직전 체결가 < 트리거가(상한가 −3호가) ≤ 이번 체결가 **재돌파** · 매수체결 · **재돌파 체결 시점 누적 거래대금 ≥ 500억**(시간대 조건 아님) · 체결강도 ≥ 150 → `guard.can_enter`(래치 없음, 활성 < 2).
> 감시 생성은 09:10~14:30 이지만 **발화 자체엔 시간 상한이 없다**(14:30 이후 재돌파도 발화).
>
> | 종목 | 사실 |
> |---|---|
> | 052710 아모텍 | 10:07 감시 시작 때 이미 잠김 → 10:59 풀림(14,600) → **11:04 15,110 으로 트리거 15,100 재돌파** → 거래대금 355억 < 500억 무산. **기록 없음** |
> | 356680 엑스게이트 | 2,055억으로 유일하게 통과. 10:40 16,030 재돌파 후 잠김 — 체결강도·매수체결 여부는 기록이 없어 불명 |
> | 나머지 8 | 6.6억~359억, 잠긴 채 감시 시작(재돌파 자체 없음 — 설계) |
>
> `CCLD_DVSN=="1"`=매수 매핑은 정확(위키독스 `3:장전, 5:매도`). **사용자 결정: 500억 하한 유지, 기록만 추가.**
> **수정 ③**: `CommandKind.GATE_FAILED` — 첫 실패 재돌파에 세션당 1회 `limit_up_event(trigger_gate_failed)` + WARNING `상한가 진입 보류`,
> 다이제스트 세션 `gate_failed`. 진입 로직·하한 불변, 마이그레이션 없음. 테스트 3건(도메인·서비스·다이제스트).
> 기록만: `시세 처리 적체 5x~7x건` 16회(10종목 동시 감시, 펌프 지연) · `feed_disconnected` 래치 중 FIRE_NET 이 오면 `daily_guard` 로 영구 종료(정각 2~6초 창).
>
> **배포 ③ `e541e6d` (11:18 KST restart).** 전체 `1241 passed`. 구 프로세스 드레인 30초 뒤 11:18:27 기동 → **11:21:02 `Application startup complete`**
> (155초 — 09:58 배포는 40초, 10:07 배포는 1초. 기동 중 `broker_sync` 2회가 먼저 돌았다. 세션 10건 복구·재구독과 `EGW00201` 재시도가 겹친 것으로 추정, 미확인).
> 재시작 후 세션 10건 전부 `watching` 유지, `trigger_gate_failed` 0행(아직 재돌파 없음). 첫 확인은 `journalctl -u maps | grep "진입 보류"`.
>
> ### `trigger_gate_failed` 첫 실측 (11:46·11:57) — 오류 아님, 설계대로
>
> | 시각 | 종목 | 트리거 | 거래대금 | 체결강도 | 매수체결 | 미달 |
> |---|---|---|---|---|---|---|
> | 11:46:51 | 443670 (#12) | 4,710 재돌파 @ 4,710 | 391억 | 110.75 | True | turnover, strength |
> | 11:57:03 | 065170 (#16) | 2,705 재돌파 @ 2,705 | 47억 | 142.05 | True | turnover, strength |
>
> 둘 다 500억·150 동시 미달. 세션은 `watching` 유지(플래그는 기록 중복만 막고 진입은 안 막는다). 오늘 세션 14건(#5~#18) 전부
> `watching / automatic`, 이벤트는 이 2행뿐, `limit_up_v1%` order_log 는 **가동 이후 여전히 0건**.
>
> **배포 ④ `11fa34d` (14:05 KST, 화면만).** 원장 이벤트 표(`static/js/app.js`) — `created_at` 이 UTC naive 라 `Z` 붙여 `Asia/Seoul` 로 표시,
> payload JSON 열을 이벤트별 한글 요약으로 교체(`거래대금 391억 (500억 미달), 체결강도 110.75 (150 미달)`), 원본 JSON 은 셀 title.
> 액션 18종·사유 코드 11종 라벨, 미등록 액션은 `key=value` 폴백. 서버·DB 불변. 전체 `1241 passed`. `analyze` 미실행 확인.
> 기동 14:05:19 → **14:08:30 `Application startup complete`**(191초 — 배포 ③의 155초와 같은 양상, `broker_sync` 가 기동 중 선행. 원인 미확인 계속).
> 재시작 후 세션 13건 `watching` 유지, 14:26 스캔이 1건 추가(#18 0011A0). 정적 JS 라 강력 새로고침 필요.
>
> 기록만: 오늘 `EGW00201` 13회(14:30 기준) — 감시 종목이 늘수록 종목당 `inquire-price` 가 늘어 악화되는 구조.
> `halted=2` 는 51 관리종목(라벨 오분류, 별건).
>
> ### 막다른 길
> - 등락률순위를 필터 없이 받아 코드에서 ≥25 거르는 방식 — 30행이 정렬돼 있지 않아 누락된다.
> - 거래량순위를 추가로 불러 "순위 0건" 구분을 유지하는 것 — 5초마다 요청 1개가 늘어 `EGW00201`(오늘도 09:03·09:12) 을 키운다. 안 했다.
>
> ### 다음 확인
> 1. ~~배포 후 첫 스캔 N ≥ 1, `limit_up_session` 당일 행~~ → **✅ 14건 생성.** 🔴 첫 `automatic` 실주문은 **여전히 없음**(9/7 이월).
>    게이트 통과 재돌파가 한 번도 없었다 — `trigger_gate_failed` 분포(거래대금 500억)가 쌓이면 하한 완화는 사람이 판단.
> 2. +25% 종목이 30개를 넘는 날의 잘림 — 오늘 최대 14건이라 미실측. `FID_INPUT_CNT_1`/`tr_cont` 페이징 판단 보류.
> 3. ~~9/9 이월: 19:00 장마감 리포트 정규 수신~~ → **✅ 9/9 19:00:29 `sent=true` 1,097자, 9/10 19:00:26 1,034자.**
> 4. 🔴 9/3 이월 validation 비교 — 9/8·9/9·9/10 모두 `evaluated 8, passed 0, failed 8` 동일. `e1a196e` 전후 수치 비교 여전히 미기록.
> 5. 기동 지연(155초·191초) 원인 — 기동 중 `broker_sync` 선행. 미확인.
> 6. `EGW00201` 레이트리밋(오늘 13회) — 미해결.
...
>
> ### 목표
> 오늘 상한가 매매 로그 점검 → **엔진이 8/29 가동 이후 후보를 한 건도 수락한 적이 없다**는
> 사실을 발견하고 근본 원인을 고쳐 배포했다(`257f6f5`, 10:35 KST).
>
> ### 진단 — 래치가 아니라 상장일 데이터 부재였다
>
> | 관측 | 값 |
> |---|---|
> | `limit_up_session` 누적 | **0건** (8/29~9/7) |
> | 스캔 탈락 99건(9/3 요약 로그 도입~) | 사유 **전부** `ineligible_security` |
> | `security_metadata.listing_date` | **2,790행 전부 NULL** |
> | 래치 없던 9/4·9/7 | 그래도 0건 |
>
> pykrx ticker-list 는 상장일을 주지 않아 `krx_adapter.get_security_meta` 가 `None` 을 적재하고,
> `collector._upsert_meta` 가 매일 None 으로 덮어썼다. `is_v1_eligible_security` 는 상장일 NULL 을
> fail-closed 로 막는다 → **삼성전자도 자격 미달**. 스캔 로그가 사유를 `ineligible_security`
...
> | | 내용 |
> |---|---|
> | R1 | `krx_adapter.fetch_listing_dates()` — pykrx 내부 `전종목기본정보().fetch("ALL")`([12005], 로그인 세션 재사용) 1회로 전 종목 `LIST_DD`. `get_security_meta` 가 싣고, `_upsert_meta` 는 None 으로 되돌리지 않는다 |
> | R2 | `runtime.v1_ineligibility_reason()` — 탈락 키가 `ineligible_security:<unknown_security|not_common_stock|listing_unknown|too_new|preferred>` |
> | R3 | 수집 후 상장(비상폐) STOCK 의 상장일 결측 비율 로그 — 절반 초과면 WARNING |
...
> ### 통한 것
>
> - **"래치 때문" 가설을 래치 없는 날(9/4·9/7) 0건으로 죽였다.** 그 뒤 DB 를 봤다.
> - **소스 후보를 운영 서버에서 1회씩 실측했다.** KIS `search-stock-info` 는 모의투자 TR 아님(`EGW02006`),
>   KRX 비로그인 POST 는 `LOGOUT`, pykrx 로그인 세션은 2,873행 성공 — 문서만 보고 정했으면 KIS 로 갔다가 막혔다.
> - 백필은 dry-run 이 예측(2,790−26=2,764)과 정확히 맞는 것을 보고 `--apply` 했다.
>
> ### 막다른 길 — 반복하지 말 것
>
> 1. 🔴 **KIS `search-stock-info`(CTPF1002R) 는 모의투자 계정에서 안 된다.** 운영은 `kis_real_trading=False`.
> 2. 🔴 **KRX `getJsonData.cmd` 를 직접 POST 하지 말 것** — 비로그인은 `LOGOUT`. pykrx 의 `전종목기본정보` 를
>    `ensure_krx_login_guard()` 뒤에서 쓴다(루트 CLAUDE.md 제약 8).
> 3. `collect_daily` 를 장중에 돌려 메타를 채우려 하지 말 것 — 미완성 봉이 OHLCV 에 들어가고
>    `has_adjusted_price` 까지 덮어쓴다. 그래서 상장일 전용 스크립트를 만들었다.
>
> ### 다음 할 일 (우선순위)
>
...
> | | 내용 |
> |---|---|
> | R6 | `scheduler.py:1535` 가 상장일 NULL 을 `2000-01-01` 로 대입 — 일일 후보 파이프라인의 상장연령 게이트(`MIN_LISTING_DAYS=100`)가 사실상 꺼져 있다. 이제 값이 채워졌으니 대입을 없애면 게이트가 살아난다(유니버스가 바뀜) |
...
> 2. ~~분석 픽 경로에는 아직 손절폭 상한이 없다~~ → **✅ 9/3 17:50 `08a74a8` 로 해소**(무장 시 `STOP_TOO_WIDE` 거부).
> 3. ~~`automatic` 첫 실주문 관측 — 래치 없는 거래일이 와야 검증된다~~ → **❌ 전제가 틀렸다(9/7).**
>    래치 없던 9/4 도 0건. 진짜 병목은 `listing_date` 전부 NULL 이었다 → 맨 위 9/7 절.
...
> ~~1번이 제일 급하다~~ → 9/3 17:50 해소. 진입 게이트(코스닥 지수 하나로만 판정) 논의는
> 아래 9/3 절 "실측이 드러낸 약점 4가지" 에 있다.
...
| | 내용 |
|---|---|
| 1 | **4거래일 중 3일 발동**(9/1 09:33, 9/2 13:04, 9/3 09:09). 거의 매일 켜지는 안전장치는 사실상 꺼진 스위치다 — ~~`automatic` 검증 자체가 불가능하다~~ **(9/7 정정: 검증을 막은 건 래치가 아니라 상장일 NULL 이었다. 래치 없는 9/4 도 0건.)** |
