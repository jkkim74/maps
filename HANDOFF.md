...
| | 내용 | 현재 상태 |
|---|---|---|
| 1 | **분석 워치리스트 픽(`strategy_trade`)은 `effective_stop_price` 를 안 쓴다.** `pick.stop_price`(LLM 산출 절대가)만 보고 **손절폭 상한 검증이 어디에도 없다**. `strategy_trade_plan.py:186-194` 는 `target > e1 > e2 > e3 > stop` 순서만 본다. 루트 `CLAUDE.md` 제약 7번 위배 | 🔴 운영 `MAPS_STRATEGY_TRADE_ENABLED=true` — **활성** |
