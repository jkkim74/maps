...
## 8/27 자동매수 유동성 하한 — ✅ 구현·검증 완료, ⏸️ 미배포
...
> ⏸️ **아직 배포하지 않았고 `master` 병합도 하지 않았다.** 운영 HEAD 는 `4f79f73` /
> Alembic `0028_holding_regime_audit` 그대로다. 마이그레이션이 없어 배포는 병합 후
> `git pull` + `systemctl restart` 만으로 끝나지만, 16:00~16:45 KST analyze 창은 피한다.
>
> 🟡 **배포 후 첫 주문일(08:55)에 확인할 것:** 예정 주문 화면의 유동성 열이 채워지는지,
> `TURNOVER_UNAVAILABLE` 로 **전량 차단되지는 않는지**. fail-closed 라 데이터 사고 시
> 매수가 전부 멎을 수 있고, 그것이 2026-08-14 사흘 차단과 같은 실패 양상이다.
