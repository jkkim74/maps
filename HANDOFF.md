...
>
> ### 다음 확인
> 1. 🔴 배포 후 첫 스캔(09:10~14:30)에서 `상한가 스캔 — 순위 N건 → 후보 N건` N ≥ 1, `limit_up_session` 당일 행, 첫 `automatic` 실주문(9/7 이월).
> 2. +25% 종목이 30개를 넘는 날의 잘림 — 실측 뒤 `FID_INPUT_CNT_1`/`tr_cont` 페이징 판단.
> 3. 9/9 이월: 19:00 장마감 리포트 정규 수신, 9/3 이월 validation 비교.
