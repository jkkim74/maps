...
>
> ### 목표
> "상한가 V1 이 왜 진입·체결을 못 하나 — 기준이 높은 건지 장세/팩터 문제인지" 를 운영 로그·DB 로 확정.
>
> ### 결론: 기준이 높은 게 아니라 **두 게이트가 동시에 만족될 수 없다**
>
> | 항목 | 실측 (9/15~9/21, 66세션) |
> |---|---|
> | 트리거 재돌파 | **481회** — 전부 게이트 평가를 받았다 |
> | 게이트 통과 | **0회** (`trigger_at` 전부 NULL, 종료 사유 전부 `no_trigger`) |
> | `limit_up_order_leg` | **0행** (가동 이후 전체) |
> | 강도 ≥150 통과 표본의 최대 거래대금 | **198억** (500억 필요) |
> | 거래대금 ≥500억 통과 표본의 최대 강도 | **120.6** (150 필요) |
> | 둘 다 통과 | **0건 / 35 관측표본** |
> | 일중 *최댓값* 기준(동시 달성 불가능한 상한) | 66세션 중 **2건(3%)**, 그중 교차 있었던 건 1건 |
>
> `ACML_TR_PBMN`(누적 거래대금)과 `CTTR`(체결강도)은 **둘 다 일중 누적값**이고, CTTR 은 누적
> 매수/매도 비율이라 거래가 쌓일수록 100 으로 수렴한다 — **거래대금이 커지면 강도는 반드시
> 내려간다.** 한 틱에서 둘 다 요구하면 교집합이 사실상 공집합이다.
>
> 모집단도 안 맞는다: `historical_ohlcv` 기준 2026년 +25% 일봉 2,590건 중 **하루 전체**
> 거래대금이 500억을 넘는 건 24.8%(중앙값 102억)뿐인데, 게이트는 그 금액을 *교차 그 순간*의
> 누적치로 요구한다. 주문은 200만원 고정이라 100억 종목에서도 체결충격은 0.02% —
> 하한이 실제 유동성 위험 대비 과도하다.
>
> ### 🔴 9/14 가설은 반증됐다
> "첫 교차만 기록해 표본이 장 초반에 쏠렸다 → 재평가하면 통과할 것" 은 **틀렸다.**
> `domain.on_trade` 는 **교차마다 재평가**하고 래치(`gate_failure_reported`)는 *이벤트 행과
> 로그만* 억제한다. 장중 전 시간대 481회를 평가하고 전부 탈락했다 — 래치를 고쳐도 0 그대로다.
>
> ### 원인이 아닌 것 (확인 완료)
> `listing_date` NULL(9/7 해소, 스캔 정상) · 코스닥 낙폭 래치(9/15 하루만) · 장세(매일 감시
> 5~18종목) · 피드 장애(`observed_tick_count` 수천~13만) · 우선주·거래정지 필터(의도대로) ·
> 모드/스위치(`enabled=true`, `automatic`).
>
> ### 관측 사각지대 2개
> 1. 481회 평가 중 **35회(7%)만 기록** — 세션당 1회 래치
> 2. `limit_up_tape` **0행** — `_dump_tape()` 호출 지점이 전부 게이트 **통과 이후** 전이
>    (TRIGGER/FIRST_FILL/LOCKED/CLOSED/EOD_DECISION)라, 한 번도 통과 못 한 엔진은 틱 증거가 없다
>
> ### 한 일 — Phase 1 계측 (`fba03c9`, 진입 판정 불변)
> - `limit_up_session.cross_samples` (JSON, 마이그레이션 `0035`): 교차마다
>   `{at, kst, price, turnover, strength, buy, failed}` 1건, 최대 200건(넘으면 **버린다** —
>   앞을 밀어내면 장 후반으로 도로 치우친다)
> - 게이트 탈락 보고를 세션당 1회 → **최대 3회·최소 5분 간격** (`_GATE_REPORT_LIMIT` /
>   `_GATE_REPORT_MIN_GAP_SECONDS`). 이벤트 행은 멱등키가 여전히 1행으로 묶는다
> - 게이트 탈락 시 직전 60초 `limit_up_tape(transition="GATE_FAILED")` 덤프
> - `TradeEvent.at_kst` — 단조시계는 재시작을 넘으면 뜻을 잃는다
> - 함께: alembic head 표기 `0033` → `0035` 정정 (코드는 이미 0034였다)
>
> ### 배포 확인 (13:04 KST)
> `alembic upgrade head` → `0035` 적용, 기동 68초 후 `상한가 V1 기동: mode=automatic`,
> `Application startup complete`. `cross_samples` 가 `[]` 로 쓰이는 것 확인(NULL 아님).
> 틱이 멈춘 세션은 기존 가드(`observed_tick_count` 동일이면 skip)가 정상적으로 건너뛴다.
>
> ### 추가 — 스카이랩스(386380) 미감시 원인과 조치 (`f7e5cbb`, 14:34 배포)
>
> "왜 상한가 관찰에 안 들어갔나" 조사. 게이트가 아니라 **자격 단계 제외**였다.
>
> | | |
> |---|---|
> | 원장 | `scan_rejections` (9/21) = `{"386380": "ineligible_security:too_new"}` |
> | 로그 | 10:42:57~13:58:12, 스캔 요약 **44회**에 `too_new=1` |
> | 코드 | `runtime.py:177` — `(as_of - listing_date).days < 100` |
> | 사실 | 상장 **2026-09-04**, 17일차. 자격 취득 **2026-12-13** |
>
> **이 게이트는 이 엔진의 입력과 무관하다.** 상한가·상장주식수·누적거래대금·체결강도는
> 전부 실시간이고 하드스톱은 ATR 없이 고정 5%(`_stop_for`) — 100일치 이력이 필요한 계산이
> 하나도 없다. `100` 은 `universe_filter.MIN_LISTING_DAYS`(일봉 지표·백테스트용)를 **상수
> 공유 없이 리터럴로 복제**한 값이고, 상한가 설계서에는 신규상장 언급이 아예 없다.
> 원설계가 아니라 9/7 상장일 적재 커밋(`257f6f5`)에 얹혀 들어왔다.
>
> **유동성 하한과 정면으로 충돌한다.** 2026년 +25% 일봉:
>
> | | 건수 | 거래대금 ≥500억 | 중앙값 |
> |---|---|---|---|
> | `too_new` (<100일) | 42 | **81.0%** | **1,179억** |
> | 자격 있음 (≥100일) | 2,534 | 24.0% | 97억 |
>
> 9/21 후보 5종목 거래대금: **386380 2,041억**(too_new 제외) · 223250 184억 · 033170 33억 ·
> 393210 16억 · 049470 10억 — **그날 하한을 넘긴 유일한 종목이 스캔 단계에서 빠졌다.**
> 단, 오늘 386380 은 저가 19,610 → 상한가 25,250(트리거 25,100)이고 재돌파 여부는 일봉으로
> 알 수 없다. 상한가에 잠겨 있었다면 어차피 진입은 없다 — **"놓친 매매"로 단정하지 않는다.**
> 진짜 손실은 감시를 안 해서 `cross_samples`·테이프가 **하나도 없다**는 것이다.
>
> ### 조치 — `too_new` 를 탈락이 아니라 **관측 전용 감시**로
> - `Candidate.observe_only` / `LimitUpMachine.observe_only`. 게이트를 통과해도 **그물을
>   던지지 않는다** — 발사하면 가상 세션이 동시 슬롯(2개)과 시도 횟수(5회 래치)를 먹어
>   **실매매를 막는다**. 통과 사실은 `cross_samples` 의 `failed == ""` 가 남긴다
> - 세션 `execution_mode="observe_only"`. 주문 경로는 전부 `== "automatic"` 비교라 fail-closed.
>   `LimitUpMode` Enum 에는 **넣지 않는다**(설정으로 고를 수 있으면 전 종목을 조용히 관측
>   전용으로 돌려 놓고 도는 줄 아는 상태가 된다)
> - 정본은 후보가 아니라 **세션 행**이다 — `watch_candidate` 도 `recover()` 도 행에서 읽어
>   재기동·같은 날 재감시에서 성격이 바뀌지 않는다
> - 정지 종목이 상장일 탓으로 기록되던 것도 함께 고쳤다
>
> ### 계측 실효 확인 (9/21 장중)
> 223250 교차 49회 중 **42건**, 425040 **20/20** 기록(13:04 배포 이후분). 종전이면 로그 2줄이
> 전부였다. `limit_up_tape` 첫 행(`GATE_FAILED`, 틱 259건)도 생겼다.
> 14:34 재기동 시 `issue_websocket_approval_key` 1회 실패(ERROR+traceback)했으나 백오프
> 재연결로 즉시 복구, 이후 ERROR 0건·틱 정상.
>
> > `observe_only` 는 스캔 진입 창(09:10~14:30)이 닫힌 뒤 배포돼 **9/22 장부터** 효력이 있다.
>
> ### 다음 할 일
> 1. 🔴 **10거래일 이상 `cross_samples` 축적 후** (거래대금, 강도) 결합분포로 임계값 결정.
>    사용자 확정 방향: **거래대금 하한 100억**, **체결강도는 최근 구간 강도로 교체**
>    (`SHNU_CNTG_SMTN`/`SELN_CNTG_SMTN` 델타 — `feed.py` 가 이미 파싱 중이라 추가 수신 불필요).
>    하한은 `domain.MIN_TURNOVER_FLOOR_KRW` + `LimitUpConfig.__post_init__` + `settings.py` 의
>    `ge` 를 **함께** 내려야 바뀐다(의도적 3중 안전장치)
> 2. 임계값 변경 **전에** 축적된 `limit_up_tape` 로 "새 기준이면 하루 몇 건인가" 를 오프라인
>    재생으로 확인. 이 전략은 일봉 백테스트가 성립하지 않아 WFA/MC/Plateau 를 못 태우므로
>    이 재생이 그 자리를 대신하는 최소 검증이다
> 3. 이번 배포에 미배포분 `5b4138f`(시장지표 캐시)가 함께 올라갔다 — **다음날 08:55 적체 0** 확인
> 4. 🟡 로컬 `tests/test_krx_adapter.py` 2건 실패는 이 PC pykrx 버전에 `전종목기본정보` 속성이
>    없어서다. 수정 **전** clean tree 에서도 동일하게 실패하며 운영 백필은 성공했다 — 별건
> 5. 🟡 `observe_only` 세션이 9/22 부터 실제로 생기는지 확인 — `execution_mode='observe_only'`
>    행이 만들어지고 주문이 **0건**인지. Phase 2 때 신규상장 허용 여부를 이 데이터로 정한다
> 6. R7(우선주 `security_type`)과 별개로, `runtime.py:177` 의 `100` 리터럴은 남아 있다 —
>    신규상장을 계속 매매에서 제외한다면 `universe_filter.MIN_LISTING_DAYS` 를 import 할 것
> 7. 이월: 첫 `automatic` 실주문 관측 · 시간외 `ORD_DVSN=21` 실검증 · R6 · `EGW00201`

> ## 9/17 세션 요약 — 상한가 시세 적체 원인 규명 → 시장지표 캐시 (`5b4138f`, 푸시만·**미배포**)
>
> ### 목표
> 오늘 상한가 세션 점검 중 발견한 "상한가 시세 처리 적체" WARNING 이 무엇인지 — 08:5x 76회, 장중 산발.
>
> ### 오늘 상한가 세션 (11:10 기준, 관측 컬럼 첫 활용)
> 감시 10건(#67~#76) 전부 `watching`, `trigger_gate_failed` 3건(285800·478340·396270, 전부 turnover+strength),
> `limit_up_order_leg` 0, `limit_up_v1%` order_log **가동 이후 0건**. 스캔 탈락 `092600 halted`.
> 478340 교차 **90회**인데 최대 거래대금 187억·체결강도 78 — 첫 교차만 남기는 `gate_failed` 로는 안 보였을 분포.
> 348080 은 틱 138·저가=고가=상한가(이틀 연속 잠김). 트리거 넘은 종목의 거래대금 최대 187억 — 500억 근처 전무.
> ERROR 0, `EGW00201` 0(9/11 은 13회).
>
> ### 진단 — 적체는 두 종류이고 둘 다 브로커 탓이 아니다
> 9/14~9/17 적체 61묶음을 지속시간·최대건수로 분류했다(`journalctl -o short-iso-precise`).
>
> | 유형 | 모양 | 원인 | 근거 |
> |---|---|---|---|
> | **멈춤형** 4건 | 3~5초간 건수 단조 증가, 해소 직후 일괄 처리 | `feeds._market_observations()` — 253거래일×전 종목 **671,643행**을 ORM 튜플로 받아 파이썬 루프 → GIL 3~4초, 같은 프로세스 상한가 펌프 정지 | 적체 마지막 줄이 `Investor flow coverage` 로그 15~30ms 전. 08:55 `order_cycle` 3일(9/15~17) + **9/14 13:16:57 `/api/v1/orders/preview` 화면 조회** |
> | **몰림형** 50여 건 | <0.1초(최대 762건/0.73초), 곧 해소 | 실제 시세 폭주. 펌프가 이벤트마다 `to_thread` 1회라 처리량 **약 2,300건/초**(실측) | 동시각 다른 활동 없음, 재돌파 `진입 보류` 와 같은 초(9/15 09:48 437730, 9/16 09:15 049080) |
>
> 08:55 가 9/15 부터인 이유: 9/9 만료 수정 이후 전일 감시 세션 13~18개가 08:59:30 만료 전까지 구독돼
> 장전 호가가 들어온다(9/1~9/14 08:5x 적체 0). 08:55 멈춤 자체는 무해 — 장전·감시 전용, 08:59:30 청산 창과 안 겹침.
>
> ### 수정 — `5b4138f` (+ HANDOFF `52805de`), 테스트 `1250 passed`
> `_market_observations()` 결과를 프로세스 캐시. 키 = `(ref_date, 창 시작일, 창 길이, count, sum(volume), sum(close))`
> — DB 가 계산하는 창 집계라 수집·보정으로 데이터가 바뀌면 저절로 재계산, 같은 키면 입력이 같다(날짜 무효화 규칙 불필요).
> 최근 8키, 락 보호. 본 계산은 `_compute_market_observations()` 로 분리. 문서 `maps/market/CLAUDE.md` feeds 절.
> 테스트 2건: 불변 데이터 재호출 시 가격 행 SELECT 0회(수정 전 1회로 RED 확인) · 기준일 행 추가 시 재계산.
> 운영 실측: 키 집계 **0.41초**(+날짜 조회 0.13초) — Postgres 대기라 GIL 을 잡지 않는다. 마이그레이션·requirements 없음.
>
> ### 통한 것
> - 적체 건수 **시퀀스**를 뽑아 "단조 증가 수 초"(멈춤)와 "수십 ms 급증 후 해소"(유입)를 가른 것. 분 단위 집계만 봤으면 broker_sync 와 엮었다.
> - nginx `access.log` 로 웹 요청을 대조 — 9/14 13:17 멈춤이 주문 예정 화면 조회였음을 찾았다.
>
> ### 막다른 길 — 반복하지 말 것
> 1. 🔴 **"broker_sync 가 느려서 적체"** — broker_sync 는 **매분 24~37초가 평소**(모의 KIS 페이서 0.5초/건을 상한가 엔진
>    지수조회 1/s·스캔이 같은 차선으로 공유, `kis_adapter._pace_request` 프로세스 전역). 늘 돌고 있어 겹쳐 보일 뿐 시각이 안 맞는다.
> 2. 경고 문구 "브로커 응답이 느리다" 를 믿지 말 것 — 두 유형 모두 브로커와 무관.
> 3. 장중에 운영 서버에서 `_market_observations` 본 계산을 재현하지 말 것 — 2 vCPU 라 그 자체가 엔진을 흔든다.
>
> ### 다음 할 일
> 1. 🔴 **배포**: 15:40~16:00 또는 16:45 이후(16:00~16:45 금지). 마이그레이션 없음. 확인은 다음날 08:55 적체 0 —
>    단 재시작 직후 첫 계산은 캐시가 비어 여전히 3~4초. `journalctl -u maps --since '08:50' | grep -c 적체`.
> 2. (선택) 시세 이벤트 묶음 처리 — 큐에 쌓인 이벤트를 `to_thread` 한 번에. 몰림형 지연(0.1~0.7초, 재돌파 순간) 감소. 핵심 경로라 보류.
> 3. (선택) 적체 WARNING 에 최고령 이벤트 지연(`now - received_at`) 추가, 문구 수정.
> 4. broker_sync 매분 24~37초 — 페이서 차선 과구독. 상한가 지수조회 1/s 가 차선 절반을 쓴다. 별건, 영향 미조사.
> 5. 9/14 절 이월: 관측 컬럼 10거래일 축적(~9/29) 후 500억 하한 판단 · 072950 "거래대금 크고 교차 0" · 9/11 이월 항목.
