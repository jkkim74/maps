...
`https://magable.kr/api/telegram/webhook` 으로 전달하도록 설정한다. secret_token을 함께
등록해 서버측(maps.api.telegram)에서 검증한다.
...
사용법 (프로젝트 루트, .env에 TELEGRAM_* 설정 후):
    ...
    python scripts/setup_telegram_webhook.py --url https://magable.kr/api/telegram/webhook
"""
...
_DEFAULT_URL = "https://magable.kr/api/telegram/webhook"
