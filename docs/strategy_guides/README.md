...
`maps/api/blog.py` + `templates/blog.html` 에 **무의존 마크다운 렌더러와
경로순회 차단**이 구현되어 있다(일일 블로그용). 같은 방식으로
`docs/strategy_guides/` 를 읽으면 새 렌더러가 필요 없다.

단, 원고가 `.txt` 순수 텍스트이므로 마크다운 변환 없이 `<pre>` 로 그대로 보여주는 편이
네이버 붙여넣기 결과와 화면이 일치해서 더 낫다.
